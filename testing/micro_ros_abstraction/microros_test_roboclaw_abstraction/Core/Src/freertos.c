...
void roboclaw_subscription_callback(const void * msgin)
{
    ...
    if (msg->data.size >= 6) {
        int32_t angle1 = msg->data.data[0];
        int32_t angle2 = msg->data.data[1];
...
void StartDefaultTask(void *argument)
{
  /* USER CODE BEGIN StartDefaultTask */
   ...
   //	      "cubemx_publisher");
   	 ...
   	 // 2. Setup the memory for the array (Important!)
  // 	volatile int32_t target_angles[6];

   	 static int32_t memory_pool[6]; // Create a buffer for 6 integers
   	 msg.data.data = memory_pool;
   	 msg.data.size = 0;
   	 msg.data.capacity = 6;

   	 // 3. Initialize the Subscriber with the correct Type Support
   	 rclc_subscription_init_default(
   	     &subscriber,
   	     &node,
   	     ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32MultiArray), // Changed from Int32
   	     "/micro_ros_roboclaw");

   	 // 4. Add to executor (msg is passed here so the executor knows where to write data)
